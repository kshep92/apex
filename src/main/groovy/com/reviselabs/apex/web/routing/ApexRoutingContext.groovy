...
class ApexRoutingContext implements ApplicationContextContainer {
    ...
    @Inject
    ApexRoutingContext(@Named("templates") String templatesFolder, TemplateEngine templateEngine) {
    ...
    void render(String template, Map data = [:]) {
        ...
        templateEngine.render(this, "$templatesFolder/$template", { result ->
            ...
            if(result.succeeded()) {
                response.setStatusCode(200).putHeader("content-length", String.valueOf(result.result().length())).end();
            } else {
